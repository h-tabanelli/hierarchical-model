...
def _run_one_alpha(
    *,
    ...
    # pre = sigma(Wx)
    # post = projection on selected directions
    Z_dbg = (S_dbg @ Vhat_f32.T).detach()

    # auxiliary object: centered + whitened projected latents
    ...
    eps_white = 1e-8
    W_white = evecs_z @ torch.diag(torch.rsqrt(torch.clamp(evals_z, min=eps_white))) @ evecs_z.T
    ...
    # selected Ritz eigenvalues on the learned subspace
    CQ_sel = estimators.C_apply_vec(rf_stream, Vhat, m=rf_width, n_total=n, device=device)
    ...
    # full raw spectrum: C = (1/n) sum_mu y_mu s_mu s_mu^T
    C_full = torch.zeros((rf_width, rf_width), dtype=torch.float64, device="cpu")
    ...
    for Xb, yb in stream_fn_factory()():
        ...
        C_full += (Sb.T @ (Sb * yb[:, None])).detach().to(device="cpu", dtype=torch.float64)
    ...
    with metrics_path.open("a", encoding="utf-8") as f:
        for alpha in [float(a) for a in task["alphas"]]:
            metrics, payload = _run_one_alpha(
                ...
                batch_size=batch_size,
                n_test=n_test,
                A_mode_teacher=A_mode_teacher,
                B_mode=B_mode,
                gamma=gamma,
                g_name=g_name,
                rf_width=rf_width,
                rf_activation=rf_activation,
                n_iter_C_max=n_iter_C_max,
                oversamp_C=oversamp_C,
                device=device,
                spec_rank=int(args.spec_rank),
            )

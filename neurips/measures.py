...
def corr_second_layer_scalar(s_hat: torch.Tensor, s_true: torch.Tensor) -> float:
    ...
    Returns r^2 (squared Pearson correlation), invariant to sign.
    """
    if s_hat.dim() == 1:
        s_hat = s_hat[:, None]
    if s_true.dim() == 1:
...
def compute_h2_layer_metrics(h2_hat, h2_star) -> dict:
    """Recovery metrics for the second-layer scalar feature h2_hat vs ground-truth h2_star.

    Args:
        h2_hat:  array-like (n,) - estimated second-layer scalar feature
        h2_star: array-like (n,) - true second-layer scalar feature (s_true pre-activation)

    Returns dict:
        pearson_r    : signed Pearson correlation r in [-1, 1]
        abs_r        : |r|
        r2           : r^2  (= corr_s computed elsewhere, but explicitly labeled)
        nmse_affine  : NMSE after optimal affine calibration = 1 - r^2
                       (min_{a,b} E[(a*h2_hat + b - h2_star)^2] / Var(h2_star))
    """
    h2_hat = np.asarray(h2_hat).reshape(-1).astype(np.float64)
    h2_star = np.asarray(h2_star).reshape(-1).astype(np.float64)

    std_hat = float(np.std(h2_hat))
    std_star = float(np.std(h2_star))

    if std_hat < 1e-12 or std_star < 1e-12:
        nan = float('nan')
        return {"pearson_r": nan, "abs_r": nan, "r2": nan, "nmse_affine": nan}

    r = float(np.corrcoef(h2_hat, h2_star)[0, 1])
    r2 = r ** 2
    return {
        "pearson_r": r,
        "abs_r": abs(r),
